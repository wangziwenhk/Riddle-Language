...
namespace Riddle {
    [[maybe_unused]] GenVisitor::GenVisitor(const std::string &moduleName): Builder(globalContext) {
        ...
        const auto printType = llvm::FunctionType::get(
                ...
                true);
        const llvm::FunctionCallee printfFunc = module->getOrInsertFunction("printf", printType);
    ...
    std::any GenVisitor::visitFuncDefine(RiddleParser::FuncDefineContext *ctx) {
        auto [types, names] = any_cast<DefineArgsType>(visit(ctx->args));
        ...
        if(isClassDefine(ParentStack.top())) {
            ...
            types.insert(types.begin(), std::get<ClassNode>(ParentStack.top()).get().types);
        }

        const std::string funcPkgName = packStack.top() + ctx->funcName->getText();

        llvm::FunctionType *funcType = llvm::FunctionType::get(resultType, types, false);
        ...
        for(int i = 0; i < names.size(); i++, argIter++) {
            llvm::AllocaInst *Alloca = Builder.CreateAlloca(types[i], nullptr, names[i]);
            Builder.CreateStore(argIter, Alloca);
            varManager.defineVar(names[i], false, Alloca);
        }
    ...
    llvm::Value *GenVisitor::binaryOperator(llvm::Value *value1, llvm::Value *value2, const std::string &op) {
        ...
        if(value1->getType()->isStructTy()) {
            ...
            //基本类型处理
            if(const auto it = opMap.find(op);it != opMap.end()) {
